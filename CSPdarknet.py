...
class CSPDarknet(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.dark2(x)
        #-----------------------------------------------#
        #   dark3的输出为80, 80, 256，是一个有效特征层(64)
        #-----------------------------------------------#
        x = self.dark3(x)
        feat1 = x
        #-----------------------------------------------#
...
# CSPDarknet的特征层，接3个yolohead

if __name__ =='__main__':
    import numpy as np
    input = torch.tensor(np.random.rand(1, 3, 320, 320), dtype=torch.float32)
    output = CSPDarknet(base_channels=64, base_depth=3, phi='s', pretrained=False)(input)
    for feat in output:
        print(feat.shape)
